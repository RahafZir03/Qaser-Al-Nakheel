...
import { updateRestaurantImage, addRestaurantImage, deleteRestaurantImage } from "../../api/endpoints/restaurant";
import { useTranslation } from 'react-i18next';
import { FaTrash } from "react-icons/fa";

import noImage from "../../assets/images/No_Image_Available.jpg"
const RestaurantImage = ({ isOpen, onClose, mainImageData, restaurantId, imageUrl, secondaryImages }) => {
  ...
  const handleMainImageSubmit = async (e) => {
    ...
    setMainLoading(true);
    try {
      const response = await updateRestaurantImage(mainImageData, formData);
  ...
  const handleAddSecondaryImage = async (e) => {
    ...
    try {
      const response = await addRestaurantImage(restaurantId, formData);
